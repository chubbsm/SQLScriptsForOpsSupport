		...
		BEGIN
			...
			SELECT @sql = N'
				...
				select
					...
					CAST(100 * @reservedpages/(1.0 * @datasize) AS NUMERIC(4,1))  AS [datafiles percent allocated],
...
SELECT [Database Name] = dbs.[database name] ,
	...
	FROM #dbs dbs
	    LEFT JOIN @spaceused su
		    ON dbs.[database name] = su.[database name]
		LEFT JOIN @logspaceused lsu
			ON su.[database name] = lsu.[database name]
	UNION ALL
	SELECT dbName = '_Total',
		NULL AS db_description,
		NULL AS recovery_model_desc,
		NULL AS dbcc_last_finished_successfully,
		SUM(su.[datafiles size GB] ),
		SUM(dbs.buffer_pool_cached_gb),
		CONVERT(DECIMAL (18,2), (SUM([dbs].[buffer_pool_cached_gb]) / SUM([su].[datafiles size GB] + .01)) * 100),
		SUM([datafiles unallocated GB]),
		SUM(dbs.buffer_pool_cached_empty_space_gb),
		NULL AS [datafiles percent allocated] ,
		SUM(su.[lob reserved GB]) ,
		SUM(su.[number of datafiles]) ,
		SUM(CAST(lsu.[logfiles size MB] / 1024. AS NUMERIC(20, 1))) ,
		NULL AS [logfile percent allocated],
		SUM(su.[number of log files]),
		NULL AS [VLF Count],
		NULL AS state_desc,
	...
	FROM @spaceused su
		LEFT JOIN @logspaceused lsu
			ON su.[database name] = lsu.[database name]
		LEFT JOIN #dbs dbs ON dbs.[database name] = su.[database name]
	ORDER BY 1;	
