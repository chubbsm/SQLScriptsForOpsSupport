...
IF NOT EXISTS(SELECT * FROM tempdb..wait_batches WHERE DATEDIFF(MI, sample_time, GETDATE()) >= 5)
	BEGIN
	WITH    memory
...
ELSE
	...
	SELECT  0 AS [Cached Data MB] ,
			0 AS [Free MB] ,
			0 AS [% Free] ,
			N'Skipped because Collector took over 5 minutes to run' AS [DB Name]
	FROM    memory
	ORDER BY [Cached Data MB] DESC
	OPTION (RECOMPILE);
	END
...
IF (SELECT LEFT(@SQLServerProductVersion,
	...
	AND NOT EXISTS(SELECT * FROM tempdb..wait_batches WHERE DATEDIFF(MI, sample_time, GETDATE()) >= 5)
BEGIN
...
END
ELSE
	SELECT NULL AS [NUMA Node], NULL AS [Cached Data MB], NULL AS [Free MB], NULL AS [% Free], 'No results for SQL Server 2005 or long-running Collector' AS [DB Name];
